...
import syntacticTree.BoolConstNode;
import syntacticTree.BreakNode;
import syntacticTree.ByteConstNode;
...
import syntacticTree.LongConstNode;
...
import syntacticTree.ShortConstNode;
...
public class TypeCheck extends VarCheck {
        ...
        for (p = x.vars; p != null; p = p.next) {
            ...
            if (l != null) {
                ...
                if (l.localcount < 0) { // se for dá uma advertência
                    System.out.println("Line " + q.position.beginLine + " Column " + q.position.beginColumn + " Warning: Variable "
                            + q.position.image + " hides a class variable");
                } else { // senão, é uma variável local em outro escopo
                    System.out.println("Line " + q.position.beginLine + " Column " + q.position.beginColumn + " Warning: Variable "
                            + q.position.image + " hides a parameter or a local variable");
                }
            }
    ...
    public void TypeCheckReadNode(ReadNode x) throws SemanticException {
        ...
        if ((t.ty != STRING_TYPE) && (t.ty != INT_TYPE) && (t.ty != DOUBLE_TYPE) && (t.ty != CHAR_TYPE) && (t.ty != LONG_TYPE)
                && (t.ty != SHORT_TYPE) && (t.ty != BOOL_TYPE) && (t.ty != FLOAT_TYPE) && (t.ty != BYTE_TYPE)) {
    ...
    public void TypeCheckIfNode(IfNode x) {
        ...
        try {
            ...
            if ((t.ty != BOOL_TYPE) || (t.dim != 0)) {
                throw new SemanticException(x.expr.position, "Integer or boolean expression expected");
            }
        } catch (SemanticException e) {
            System.out.println(e.getMessage());
            foundSemanticError++;
    ...
    public void TypeCheckSwitchNode(SwitchNode x) {
        ...
        try {
            ...
            int k = 0;
            for (p = x.stat; p != null; p = p.next) {
                c = (SwitchCaseNode) p.node;
                TypeCheckSwitchCaseNode(c);
                
                if((x.def != null) && (p.node == x.def.node)) {
                        if (c.expr != null)
                            throw new SemanticException(p.position, "O comando Default não aceita uma expressão de comparação");
                } else {
                    type cexpr = TypeCheckExpreNode(c.expr);
    ...
    // ---------------------------------- comando case --------------------
    public void TypeCheckSwitchCaseNode(SwitchCaseNode x) {
        type t;
    ...
    public type TypeCheckNewObjectNode(NewObjectNode x) throws SemanticException {
        ...
        if (p == null) {
            throw new SemanticException(x.position,
                    "Constructor " + x.name.image + "(" + ((t.ty == null) ? "" : ((EntryRec) t.ty).toStr()) + ") not found");
        }
    ...
    public type TypeCheckRelationalNode(RelationalNode x) throws SemanticException {
        ...
        if ((isSubClass(t2.ty, t1.ty) || isSubClass(t1.ty, t2.ty)) && ((op == langXConstants.NEQ) || (op == langXConstants.EQ))) {
            return new type(INT_TYPE, 0);
        }
